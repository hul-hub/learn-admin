  ...
  <div class="datas-daily">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="body">
        ...
        >
          1.
          对于渠道，默认情况下看渠道每日推广业绩汇总，但可查询具体某个主渠道或子渠道每日推广业绩。
          <br />2.
          对于主渠道，默认情况下看主渠道每日推广业绩汇总，但可查询某个子渠道每日推广业绩。
          <br />3. 对于子渠道，默认且只能查看自己每日推广业绩。
        ...
        <el-table
          ...
          v-loading="loading"
        >
          <el-table-column prop="recordDate" label="日期" align="center">
...
import { daystatistics } from "@/api/datacenter";
...
export default {
  ...
  data() {
    ...
    if (this.$route.query) {
      if (this.$route.query.scLoginName) {
        defaultData.queryKey = 3;
        defaultData.queryValue = this.$route.query.scLoginName || "";
      }
    }
  ...
  computed: mapState({
    gameListarr: state => state.gameList.gameListarr
  }),
  created() {
    ...
    if (this.$route.query) {
      if (this.$route.query.scLoginName) {
        this.filters.queryKey = 3;
        this.filters.queryValue = this.$route.query.scLoginName || "";
      }
    }
    ...
    this.$store.dispatch("getUserGameList");
  },
  methods: {
    handleGamesChange(data) {
      this.filters.platformType = data.platformType;
      console.log(this.filters.platformType);
    ...
    async submitFilter() {
      ...
      res.data[res.data.length - 1].recordDate = "汇总";
      this.dataList = res.data;
    },
    getDatePickDate(queryDateTypes, queryDateValue, def = []) {
      const dateFilter = queryDateTypes[queryDateValue - 1];
      if (1 === dateFilter.id) {
    ...
    exportDatas() {
      ...
      http.get2("/datacenter/daystatistics.do", data).then(res => {
        if (res.state === "fail") {
