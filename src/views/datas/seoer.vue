  ...
  <div class="datas-seoer">
            ...
            <el-row :gutter="10">
              <el-col :xs="24" :sm="24" :md="24" :lg="10" :xl="8">
                <div class="merge">
                  ...
                  <el-select
                    filterable
                    v-model="filters0.queryKey"
                    placeholder="请选择"
                    style="width: 150px"
                    v-if="typeStatus"
                    @change="handleQueryKeyChangeName"
                  >
                    <el-option
                      v-for="item in filters0.queryKeyTypes"
                      :key="item.id"
                      :label="item.name"
                      :value="item.name"
                    />
    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="body">
        ...
        >
          1.
          对于渠道，默认查看渠道内所有子渠道的推广数据，但可查询某个子渠道或某个主渠道内所有子渠道的推广数据。
          <br />2.
          对于主渠道，默认查看主渠道内所有子渠道的推广数据，但可以查询某个子渠道的推广数据。
        ...
        <el-table
          ...
          v-loading="loading"
        >
          <el-table-column prop="scLoginName" label="子渠道账号" align="center">
            <template slot-scope="scope">{{ scope.row.scLoginName }}</template>
          </el-table-column>
          <el-table-column
            prop="scRealName"
            label="子渠道备注"
            ...
            align="center"
          >
            <template slot-scope="scope">{{ scope.row.scRealName }}</template>
          </el-table-column>
          <el-table-column
...
import { scstatistics, getGameScRealNames } from "@/api/datacenter";
...
export default {
  ...
  data() {
    ...
    if (this.$route.query) {
      if (this.$route.query.mcChannelName) {
        defaultData.queryKey = 2;
        defaultData.queryValue = this.$route.query.mcChannelName;
      }
    }
  ...
  methods: {
    ...
    async handleQueryKeyChange(option) {
      if (option === 14) {
        this.typeStatus = true;
        const nameDataList = await getGameScRealNames(); //获取子渠道备注列表
        const arr = nameDataList.data;
        const arryList = [];
        arr.forEach(function(element, index) {
          const list = { id: index + 1, name: element };
          arryList.push(list);
        });
        this.filters0.queryKeyTypes = arryList;
      } else {
        this.filters0.queryKey = "";
        this.typeStatus = false;
        this.filters.queryValue = "";
      }
    },
    //子渠道备注改变
    handleQueryKeyChangeName(option) {
      this.filters.queryValue = option;
    },
    ...
    async submitFilter() {
      ...
      res.data[res.data.length - 1].scRealName = "汇总";
    ...
    exportDatas() {
      ...
      http.get2("/datacenter/scstatistics.do", data).then(res => {
        if (res.state === "fail") {
    ...
    lookupTotal(vo) {
      this.$router.push(
        `/datas/daily?scLoginName=${vo.scLoginName}&scId=${
          vo.scId
        }&scRealName=${vo.scRealName}`
      );
    },
    canShowRow(vo) {
