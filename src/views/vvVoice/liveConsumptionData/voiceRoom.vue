  ...
  <div>
    ...
    <el-card class="box-card" shadow="never">
      <div slot="header" class="clearfix">
        <h3>语音房消费记录</h3>
        <div class="extra">
          <el-button size="medium" @click="exportLiveConsume" type="primary"
            >导出</el-button
          >
        </div>
      </div>
...
import { getLiveConsumeApi, exportLiveConsumeApi } from "@/api/vvVoice";
...
export default {
  ...
  methods: {
    ...
    exportLiveConsume() {
      ...
      const url = `/vvData/exportLiveConsume.do?${serialize(data)}`;
      exportLiveConsumeApi(data).then(res => {
        if (res.state === "fail") {
          ...
          downloadElement.download = "语音房消费记录.xlsx";
